...
Material* _CreateMaterial(const char* _mtlPath)
{
...
void _CompileShader(char* _path)
{
...
void Material::DefaultInit()
{
	char _vert[64];
	char _frag[64];
	sprintf(_vert, "%s.vert", mShaderPath);
	sprintf(_frag, "%s.frag", mShaderPath);
	Init(_vert, _frag);
}

Material::Material(char* _vert, char* _frag)
{
	Init(_vert, _frag);
}

void Material::Init(char* _vert, char* _frag)
{
